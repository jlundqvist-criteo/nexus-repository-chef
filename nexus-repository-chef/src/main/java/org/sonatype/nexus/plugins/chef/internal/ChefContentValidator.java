 ...
 * @since 0.0.1
 */
@Named(org.sonatype.nexus.plugins.chef.internal.ChefFormat.NAME)
@Singleton
public class ChefContentValidator
    ...
    implements ContentValidator
{
  ...
  @Inject
  public ChefContentValidator(final DefaultContentValidator defaultContentValidator) {
                                     ...
                                     @Nullable String declaredContentType) throws IOException
  {
    if (contentName != null) {
      if (!contentName.endsWith(".tar.gz")) {
        // Note: Tika thinks the chef json responses are text/plain since we aren't getting them as .json files
